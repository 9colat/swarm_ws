...
void data_to_be_sent (const std_msgs::String& data_msg) {
  ...
  radio_module.write(&message_to_send); //sending data
}



ros::Subscriber<std_msgs::String> subscriber("data_to_be_sent", data_to_be_sent );
ros::Publisher publisher("data_to_be_received", &data_to_be_received);
...
void receiving() {
  if (radio_module.available() > 0) { // is there something on the radio? (enable the radio)
    ...
    publisher.publish(&data_to_be_received);
  }
}
...
void loop() {
  ...
  data_to_be_received.data = "hello";
  receiving();
  nh.spinOnce();
